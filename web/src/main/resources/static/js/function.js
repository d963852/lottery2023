...
function MemberAdd() {
    ...
    if (parseFloat(this.fanDian.value) > parseFloat($(this.fanDian).attr('max'))) {
        layer.msg('返点不能大于' + $(this.fanDian).attr('max'));
        return false;
    }
    var fanDianDiff = $(this.fanDian).attr('fanDianDiff');
    if ((this.fanDian.value * 1000) % (fanDianDiff * 1000)) {
        layer.msg('返点只能是' + fanDianDiff + '%的倍数');
        return false;
    }
}

function Dinding() {
    if (this.account && !this.account.value) {
        layer.msg("请填写收款账号");
        return false;
    }
    ...
    if (this.countname && !this.countname.value) {
        layer.msg("请填写开户地址");
        return false;
    }
    if (this.coinPassword && !this.coinPassword.value) {
        layer.msg("请输入资金密码");
        return false;
    }
    if (this.coinPassword && this.coinPassword.value.length < 6) {
        layer.msg("资金密码至少6位");
        return false;
    }
    return true;
...
function BonusDaySet() {
    if (!this.coinpwd.value) {
        layer.msg("请输入资金密码");
        return false;
    }
    if (parseFloat(this.amount.value) >= 1) {
        layer.msg("团队当前为盈利，申请失败");
        return false;
    }
    if (parseFloat(this.issue.value) >= parseFloat(this.bonus.value)) {
        layer.msg("累积分红大于亏损分红，申请失败");
        return false;
    }
    if (parseFloat(this.money.value) <= 100) {
        layer.msg("本次分红低于100，申请失败");
        return false;
    }
    return true;
}
...
function LinkAdd() {
    ...
    if (!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) {
        layer.msg("请正确设置返点");
        return false;
    }
    if (parseFloat(this.fanDian.value) < 0) {
...
$(function () {
    $(':radio[name=danwei]').on("click", function () {
        var value = $(this).val();
        if ($(this).attr("checked")) {
            $.cookie('mode', value, {expires: 7, path: '/'});
        }
    })
    $("#basic_slider").on("mouseover", function () {
        $.cookie('fanDian', gameGetFanDian(), {expires: 7, path: '/'});
    })
    $('.changebg a').on("click", function () {
